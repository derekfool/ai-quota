...
- `Assets/WatchCats/surprised-open.png` and `surprised-blink.png`: owner-approved surprised pose generated with OpenAI image generation using the supplied expression and existing mascot/icon style references. Local foreground extraction removes the generated background; aligned eye-only compositing preserves the mouth and body between frames. Both images share the same crop and scale.

- `Assets/WatchCats/catnip-open.png` and `catnip-blink.png`: owner-approved wholesome catnip-daydreaming pose, generated using OpenAI image generation from the existing mascot/icon references. Original transparency and aligned eye-only compositing preserve the leaf and body; both frames share the same crop and scale.
