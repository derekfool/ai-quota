...
Each new Close Watch session draws from a shuffled bag of sad, blank, lying, angry, surprised, and catnip poses. Each pose appears once per round; the exhausted bag is shuffled again. The store owns the in-memory bag, which resets on app restart. Round boundaries may repeat a pose. The shared store holds that choice for both dashboard surfaces; refreshes do not reselect it. The mascot is decorative, hidden from accessibility, ignores pointer input, and never contributes to layout measurement.

The first provider card publishes its actual bounds. A Canvas overlay clips to the dashboard rounded outline and subtracts the first card for occlusion. Standing poses cover AI QUOTA and occupy the left gutter, using the approved nose-to-belly axis rotated by -0.32 radians for a 0.30-second entry and 0.22-second exit. Resting poses overlap the card top by 8 points, except the surprised cat at 26 points (18 points lower to expose its eyes) and the catnip cat at 18 points (10 points lower to seat both paws); on stop or expiry, it switches behind the card, waits 0.06 seconds, then retracts over 0.22 seconds. Eye-only blink frames preserve body registration. The lying cat also bends its outer right ear twice after settling, then rests between flicks (8.7-second cadence). A localized strip deformation leaves the cheek and paws fixed; both eye frames share it. All resting poses use the same 136-point width, preserving the artwork aspect ratio. The lying cat is the scale reference for future cats; overflow is clipped by the window instead of shrinking a pose to fit its ears. The angry cat's upper 40% bends both ears inward with up to 18% of the image width in tip displacement, tapering to zero above the eyes. The surprised cat uses the same dual-ear bend over its upper 32% to avoid moving the larger eyes. Both upright resting cats share the resting entry/occlusion and ear timing, while the original lying animation is unchanged. Exit suppresses ear motion. Reduce Motion shows a static pose and hides without translation, blinking, or ear motion. A cancelled exit task cannot hide a restarted session. Reordering temporarily hides the decoration to preserve the reorder controls; the chosen pose remains unchanged.
...
The surprised cat adds aligned open/blink frames; the open mouth and paws do not change during blinking. Tall ears may be outside the window clip at the approved scale, including during their animation.

The catnip pose joins the shuffle bag at the same 136-point width as the lying pose, with 18-point card overlap to accommodate its uneven paw baseline. It reuses the resting lifecycle and blink cadence. The right-ear deformation is limited to the outer 15% of the image above 80% height, keeping the leaf and paw fixed. It has no body shake.
